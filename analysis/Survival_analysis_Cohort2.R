...
cohort_censor$phq_cat[cohort_censor$phq_base<10] = 'no depressive symptoms'  # no depressive symptoms
cohort_censor$phq_cat[cohort_censor$phq_base>=10] =  'depression'  # mild
...
# Cox active
res.cox <- coxph(Surv(duration_active_in_660, event = censorStatus_active) ~  age_cat+sex +phq_base+ site +income_cat+ marry_align+edu_age_align +accommodation_cat +children_cat, data = cohort_censor)
summary(res.cox)
cox.zph(res.cox)
ggforest(res.cox,data = cohort_censor)
# Cox phone passive
res.cox <- coxph(Surv(duration_phone_in_660, event = censorStatus_phone) ~  age_cat+sex +phq_base+ site +income_cat+ marry_align+edu_age_align+accommodation_cat +children_cat, data = cohort_censor)
summary(res.cox)
cox.zph(res.cox)
ggforest(res.cox,data = cohort_censor)
# Cox fitbit
res.cox <- coxph(Surv(duration_fitbit_in_660, event = censorStatus_fitbit) ~  age_cat+sex +phq_base+ site +income_cat+ marry_align+edu_age_align +accommodation_cat +children_cat, data = cohort_censor)
summary(res.cox)
cox.zph(res.cox)
ggforest(res.cox,data = cohort_censor)
