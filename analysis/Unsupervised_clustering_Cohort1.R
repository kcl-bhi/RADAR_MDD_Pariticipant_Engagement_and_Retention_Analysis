...
# create two variable to reflect the time peiod between the participant' enrollment and the first day of the study
...
# select active data before 301 days
active.data.select <- active.data.select %>%
  dplyr::filter(daysInStudy <= 301 & daysInStudy>0) %>%
  dplyr::group_by(p_id, biweek) %>%
  dplyr::summarise(n = n() )

# biweek number is 1-22, represent week 1-2, 3-4, ..., 43-44. 
tmp <- expand.grid(biweek = seq(1,22,by=1),p_id = select_participant) # num of rows = 22*614
...
ids.data.in301days <- merge(ids.data, enrollment.info) %>%
  dplyr::filter(date - as.Date(enrollment_date)+1<=301 | record_num==0)
ids.count <-ids.data.in301days %>%
  dplyr::group_by(p_id)%>%
  dplyr::summarise(count_ids = n())

# mean of PHQ8,response, survey time, overtime
phq8.notf.cohort1<- phq8.notf %>%
  dplyr::filter(days<=301) %>%
  dplyr::group_by(p_id) %>%
  dplyr::summarise(mean_phq8_score = median(sum_phq8),mean_phq8_response_time = median(delta.notif.vs.start)/60,mean_phq8_total_time=median(total.time.taken))
rses.notf.cohort1<- rses.notf %>%
  dplyr::filter(days<=301) %>%
  dplyr::group_by(p_id) %>%
...
res.wss
ggsave(file='Figures/Cohort1_active_optimalClusterN_WSS_method.png',plot=res.wss,width = 4, height = 4, dpi = 100)

k <- 3
set.seed('3435435')
km <- kmeans(as.matrix(tmp_matrix_01), centers = k)
km.cluster.allocation <- factor(km$cluster,levels= c('2','1','3'))
new_label <- seq(1,43,2)
...
print(ht.phone.active)

png(filename = "Figures/heatmap_clusters_PhoneActiveData_Cohort1.png",
...
pid_by_clusters_wmdata <- merge(pid_by_clusters_wmdata ,phq8.notf.cohort1, all.x = T)
pid_by_clusters_wmdata <- merge(pid_by_clusters_wmdata ,rses.notf.cohort1, all.x = T)
...
sum_active_week <- merge(sum_active_week, baseline.phq8,all.x = T)
sum_active_week <- merge(sum_active_week, demog.data.flt,all.x = T)
sum_active_week <- merge(sum_active_week, baseline.ids,all.x = T)
...
listVars <- c("age", "sex","pssuq_all","pssuq_1","pssuq_2","pssuq_3", "sum_phq8", "sum_rses","delta.enrol.vs.study_start","site", "marry_align",
              "edu_age_align", "children","income_align","accommodation_occupy_align","accommodation_satisfy","count_survey","num_contact","ids_score","count_ids","mean_phq8_score","mean_phq8_response_time","mean_phq8_total_time","mean_rses_score","mean_rses_response_time","mean_rses_total_time")

#Define categorical variables
catVars <- c("sex","site", "marry_align", "children","income_align","accommodation_occupy_align","accommodation_satisfy")
tabResults <- tableone::CreateTableOne(data=pid_by_clusters_wmdata, vars=listVars, 
                                       factorVars = catVars, 
                                       strata = c("cluster"), includeNA = F)
demo_table <- print(tabResults, nonnormal = c("age", "sex","pssuq_all","pssuq_1","pssuq_2","pssuq_3", "sum_phq8", "sum_rses","delta.enrol.vs.study_start", "marry_align",
                                              "edu_age_align", "children","income_align","accommodation_occupy_align","accommodation_satisfy","count_survey","num_contact","ids_score","count_ids","mean_phq8_score","mean_phq8_response_time","mean_phq8_total_time","mean_rses_score","mean_rses_response_time","mean_rses_total_time"), 
...
write.csv(demo_table, 
          file = "Tables/Table_PhoneActiveData_Clusters_Comparison_Cohort1.csv")

#########################
# Passive data-- Phone Passive
#########################
...
passive.phone.data.select <- passive.data.flt %>%
  dplyr::filter(daysInStudy <= 301 & daysInStudy>0 & dataName=="Passive_phone") %>%
  dplyr::group_by(p_id, daysInStudy) %>%
  dplyr::summarise(n = n())

# fitbit data
passive.fitbit.data.select <- passive.data.flt %>%
  dplyr::filter(daysInStudy <= 301 & daysInStudy>0 & dataName=="Passive_Fitbit") %>%
  dplyr::group_by(p_id, daysInStudy) %>%
  dplyr::summarise(n = n())

select_participant<-demog.data.flt$p_id
tmp <- expand.grid(daysInStudy = seq(1,301,by=1),p_id = select_participant) 
...
res.wss
ggsave(file='Figures/Cohort1_phone_optimalClusterN_WSS_method.png',plot=res.wss,width = 4, height = 4, dpi = 100)
...
ht.phone.passive <- ComplexHeatmap::Heatmap(as.matrix(tmp_phone), show_heatmap_legend = F,
                                            ...
                                            show_row_dend = F, cluster_columns = F,cluster_rows = F,row_title = new_row_title,
                                            show_column_names = T, show_row_names = F,column_labels = new_label,row_title_rot = 0,
                                            heatmap_legend_param = list(labels_gp = grid::gpar(fontsize = 1)))
ht.phone.passive
png(filename = "Figures/heatmap_clusters_PhonePassiveData_Cohort1.png",
    height = 6, width = 5, res = 200, units="in")
ht.phone.passive <- draw(ht.phone.passive)
dev.off()
...
pid_by_clusters <- reshape2::melt(pid_by_clusters)
colnames(pid_by_clusters) = c('p_id', 'cluster')
passive_result <- pid_by_clusters
...
#Create a variable list which we want in Table 1
listVars <- c("age", "sex","pssuq_all","pssuq_1","pssuq_2","pssuq_3", "sum_phq8", "sum_rses","delta.enrol.vs.study_start","site", "marry_align",
              "edu_age_align", "children","income_align","accommodation_occupy_align","accommodation_satisfy","count_passive","num_contact","ids_score","count_ids","mean_phq8_score","mean_phq8_response_time","mean_phq8_total_time","mean_rses_score","mean_rses_response_time","mean_rses_total_time")
...
all<-rbind(all,ids_lon_table[2,])
write.csv(demo_table, 
          file = "Tables/Table_PhonePassiveData_Clusters_Comparison_Cohort1.csv")
...
res.wss
ggsave(file='Figures/Cohort1_fitbit_optimalClusterN_WSS_method.png',plot=res.wss,width = 4, height = 4, dpi = 100)

k <- 3

set.seed('3435000')
km <- kmeans(as.matrix(tmp_fitbit), centers = k)
km.cluster.allocation <- factor(km$cluster,levels = c('1','2','3'))
new_label <- seq(0,300)
new_label <- ifelse(new_label%%14==0, new_label/14 +1, -1)
new_label <- ifelse(new_label!=-1, new_label*2 -1, -1)
new_label <- as.character(new_label)
...
png(filename = "Figures/heatmap_clusters_FitbitPassiveData_Cohort1.png",
    height = 6, width = 5, res = 200, units="in")
...
pid_by_clusters <- reshape2::melt(pid_by_clusters)
colnames(pid_by_clusters) = c('p_id', 'cluster')
fitbit_results<-pid_by_clusters
# Link clusters with demographic,
pid_by_clusters_wmdata <- merge(pid_by_clusters ,demog.data.flt, all.x = T)
pid_by_clusters_wmdata <- merge(pid_by_clusters_wmdata ,enrollment.info, all.x = T)
...
baseline.phq8 <- phq8.notf %>% dplyr::filter(week %in% c(1,2)) %>% 
  select(p_id, sum_phq8) %>% 
  dplyr::group_by(p_id) %>%
  dplyr::summarise(sum_phq8 = slice(which.max(sum_phq8)))
pid_by_clusters_wmdata <- merge(pid_by_clusters_wmdata ,baseline.phq8, all.x = T)
...
#Create a variable list which we want in Table 1
listVars <- c("age", "sex","pssuq_all","pssuq_1","pssuq_2","pssuq_3", "sum_phq8", "sum_rses","delta.enrol.vs.study_start","site", "marry_align",
              "edu_age_align", "children","income_align","accommodation_occupy_align","accommodation_satisfy","count_fitbit","num_contact","ids_score","count_ids","mean_phq8_score","mean_phq8_response_time","mean_phq8_total_time","mean_rses_score","mean_rses_response_time","mean_rses_total_time")
...
all<-rbind(all,ids_lon_table[2,])
write.csv(demo_table, 
          file = "Tables/Table_FitbitPassiveData_Clusters_Comparison_Cohort1.csv")
...
p <- p + theme_bw(base_size = 15)  + ylab('Scores') +xlab('') + ggtitle("Baseline PHQ8")
p <- p + theme(legend.title = element_blank(),plot.title = element_text(hjust = 0.5),
               axis.text.x = element_text(size=13, face="bold")) 
p <- p + guides(fill=FALSE)
p <- p + scale_fill_manual(values=c('#2c7fb8', '#7fcdbb', '#edf8b1'))

p
ggsave("Boxplot_cohort1_baseline_phq8_compare_datastreams.png", plot=print(p), height = 6, width = 7, units="in", dpi=250)


# response phq8
p <- ggplot(data = cluster_all, aes(x=cat, y=mean_phq8_response_time, fill=cluster)) + geom_boxplot(outlier.shape = NA,width=0.5)
p <- p + theme_bw(base_size = 15)  + ylab('Minutes')+xlab('')  + ggtitle("PHQ8 response time")
p <- p + theme(legend.title = element_blank(),plot.title = element_text(hjust = 0.5),
               axis.text.x = element_text(size=13, face="bold")) 
...
p <- p + scale_fill_manual(values=c('#2c7fb8', '#7fcdbb', '#edf8b1'))
p
ggsave("Boxplot_cohort1_phq8_response_time_compare_datastreams.png", plot=print(p), height = 6, width = 7, units="in", dpi=250)
...
p <- p + theme_bw(base_size = 15)  + ylab('Seconds')+xlab('')  + ggtitle("PHQ8 completion time")
p <- p + theme(legend.title = element_blank(),plot.title = element_text(hjust = 0.5),
               axis.text.x = element_text(size=13, face="bold")) 
...
p <- p + scale_fill_manual(values=c('#2c7fb8', '#7fcdbb', '#edf8b1'))
p
ggsave("Boxplot_cohort1_phq8_total_time_compare_datastreams.png", plot=print(p), height = 6, width = 7, units="in", dpi=250)

# age
p <- ggplot(data = cluster_all, aes(x=cat, y=age, fill=cluster)) + geom_boxplot(outlier.shape = NA,width=0.5)
p <- p + theme_bw(base_size = 15)  + ylab('Years')+xlab('')  + ggtitle("Age")
p <- p + theme(legend.title = element_blank(),plot.title = element_text(hjust = 0.5),
               axis.text.x = element_text(size=13, face="bold")) 
p <- p + guides(fill=FALSE)
p <- p + scale_fill_manual(values=c('#2c7fb8', '#7fcdbb', '#edf8b1'))
p
ggsave("Boxplot_cohort1_age_compare_datastreams.png", plot=print(p), height = 6, width = 7, units="in", dpi=250)
